...
class TestDataSet(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.dataitem1 = DataItem('test_id1', pd.DataFrame({'a': [1, 2, 3], 'b': [4, 5, 6]}), pd.Series({'test id': 'test_id1', 'a': 1, 'b': 2}))
        self.dataitem2 = DataItem('test_id2', pd.DataFrame({'a': [1, 2, 3], 'b': [4, 5, 6]}), pd.Series({'test id': 'test_id2', 'a': 1, 'b': 2}))
        self.dataitem3 = DataItem('test_id3', pd.DataFrame({'a': [1, 2, 3], 'b': [4, 5, 6]}), pd.Series({'test id': 'test_id3', 'a': 1, 'b': 2}))
    ...
    def test_init(self):
        ...
        self.assertEqual(len(dataset.dataitems), 3)
        self.assertTrue(self.dataitem1 in dataset.dataitems)
        self.assertTrue(self.dataitem2 in dataset.dataitems)
        self.assertTrue(self.dataitem3 in dataset.dataitems)

    def test_get_item(self):
        dataset = DataSet(self.data_dir, self.info_path)
        dataitem = dataset['test_id1']
