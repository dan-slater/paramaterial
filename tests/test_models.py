...
class TestModelSet(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.info_table = pd.DataFrame({'test_id': ['id_001', 'id_002'], 'a': [1, 2], 'b': [4, 5]})
        ...
        self.params_table = pd.DataFrame({'test_id': ['model_id_001', 'model_id_002'], 'p1': [2, 2], 'p2': [3, 3]})
        ...
        self.results_table = pd.DataFrame({'test_id': ['id_001', 'id_002'], 'fun': [0.1, 0.1], 'success': [True, True]})
    ...
    def test_fit(self):
        """Test ModelSet.fit"""
        dataset = DataSet(self.data_dir, self.info_path, 'test_id')
        model_set = ModelSet(
            model_func=linear_model,
            param_names=['p1', 'p2'],
            bounds=[(0, 10), (0, 10)]
