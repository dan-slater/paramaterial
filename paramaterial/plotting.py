...
def dataset_plot(dataset, x: str, y: str,
    ...
    color_key = cbar_by if cbar_by is not None else color_by
    if color_key is not None and cbar_norm is None:
        cbar_norm = plt.Normalize(vmin=dataset.info_table[color_key].min(), vmax=dataset.info_table[color_key].max())
    ...
    for dataitem in dataset:
        ...
        if color_key is not None:
            color = colormap(cbar_norm(dataitem.info[color_key]))
            zorder = dataitem.info[color_key]
        else:
