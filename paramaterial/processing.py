...
def make_representative_curves(dataset: DataSet, data_dir: str, info_path: str,
    ...
    # make a dictionary with the keys as vals from the repr_by_cols
    # each value will be a list of the unique values of that column from the info table
    ...
    # make list of filters from repr_by_cols
    # find unique values of the categories in repr_by_cols from the dataset info table
    # make one filter for each unique combination of the values in each category
    # each filter should have one value for every category in repr_by_cols
    ...
    repr_info_table = pd.DataFrame(columns=['test id'] + repr_by_cols)

    # loop through representative subsets
    for repr_id, subset_filter in zip(repr_ids, subset_filters):
        ...
        info_row = pd.Series({'repr id': repr_id, **subset_filter})
        repr_info_table = pd

        # find minimum of maximum interp_by vals in subset
        max_interp_val = min([max(data_item.data_table[repr_col]) for data_item in repr_subset])
        # make monotonically increasing vector to interpolate by
        interp_vec = np.linspace(min_interp_val, max_interp_val, interp_res)
        # dataframe for storing interpolated data to be averaged
        interp_data = pd.DataFrame(data={interp_by: interp_vec})

        # loop through dataitems in representative subset
        for n, dataitem in enumerate(repr_subset):
            # drop columns other than repr_col and interp_col from dataitem data
            data = dataitem.data[[interp_by, repr_col]].reset_index(drop=True)
            # drop rows outside of interp range
            data = data[(data[interp_by] <= max_interp_val) & (data[interp_by] >= min_interp_val)]
            # interpolate the repr_by column
            interp_col = np.interp(interp_vec, data[interp_by], data[repr_col])
            # add interpolated col to interp_data
            interp_data[f'interp_{repr_col}_{n}'] = interp_col

        # add the mean, std, min, max, q1 and q3 of the interpolated repr_col to the repr_data
        repr_data = pd.DataFrame({interp_by: interp_vec})
        ...
        repr_info = copy.deepcopy(repr_subset.info_table)
